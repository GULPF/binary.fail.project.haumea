...
// Algorithm based on "An algorithm for locating candidate labeling boxes within a polygon"
...
// NOTE: Does not take holes into account, but will still accept polygons with holes.
// ..... Callers should compare the area of the label and the polygon to try and detect bad scenarios.
namespace Haumea.Geometric
{
    public static class FindPolygonLabel
    {
        ...
        private static IList<VerticalSegment> GetVerticalSegments(this IPoly poly)
        {
        ...
        private static IList<Strip> GetStrips(this IPoly poly)
        {
        ...
        private static IList<LineEkv> GetLineEkvations(this IPoly poly)
        {
        ...
        private static void DrawDebug(this IPoly poly, IList<Strip> strips, IList<VerticalSegment> segments)
        {
            int i = 0;
            RenderInstruction[] lines = new RenderInstruction[segments.Count + strips.Count + 1];
            Vector2 start, end;
            float xMin = poly.Boundary.TopLeft.X;
            float xMax = poly.Boundary.BottomRight.X;
        ...
        public static IList<AABB> LabelBoxCondidates(this IPoly poly)
        {
            return poly.LabelBoxCondidates(false);
        }

        private static IList<AABB> LabelBoxCondidates(this IPoly poly, bool suppressDebug)
        {
        ...
        /// </summary>
        /// <returns>The best label box (hopefully).</returns>
        public static AABB FindBestLabelBox(this IPoly poly)
        {
            IList<AABB> boxes  = poly.LabelBoxCondidates(false);
            IList<AABB> rBoxes = new List<AABB>();

            // Here be dragons.
            // TODO This stopped working when I fixed the AABB
            /*foreach (AABB box in poly.RotateLeft90().LabelBoxCondidates(true))
            {
                Vector2 rmin = box.TopLeft.RotateRight90();
                Vector2 rmax = box.BottomRight.RotateRight90();
            ...
            {
                Vector2 dim = (box.BottomRight - box.TopLeft).Abs();
                if (dim.X * dim.Y > dmax)
                {
                    dmax = dim.X * dim.Y;
                    choosen = box;
